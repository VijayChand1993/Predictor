...
Formula (Phase 8 - Transit Blending + No Normalization):
effectivePlanetScore = 0.7×planetScore + 0.3×transitScore
H(h) = Σ C(p → h), capped at 100
(Pure accumulation, no cross-house normalization, transit-responsive)
"""
...
class HouseActivationService:
    ...
    def calculate_planet_house_contributions(
        ...
        """
        Calculate how a planet's score is distributed to houses (Phase 8 - Transit Blending).

        NEW (Phase 8):
        Uses effectivePlanetScore = 0.7×planetScore + 0.3×transitScore
        This ensures houses reflect current transits, not just static planet strength.
        ...
        Args:
            ...
            transit_score: The planet's transit component score (0-100)

        Returns:
            PlanetHouseContributions with distribution to all houses
        """
        # Phase 8: Blend planet score with transit score
        # This makes houses more responsive to current transits
        effective_planet_score = 0.7 * planet_score + 0.3 * transit_score
        contributions = {}
        ...
        # Calculate contributions for each house (using effective score)
        for house_num in range(1, 13):
            ...
            # Transit contribution (30%) - uses effective score
            if house_num == transit_house:
                transit_contrib = effective_planet_score * self.TRANSIT_PERCENTAGE

            # Natal placement contribution (20%) - uses effective score
            if house_num == natal_house:
                natal_contrib = effective_planet_score * self.NATAL_PERCENTAGE

            # Ownership contribution (30%, split if multiple) - uses effective score
            if house_num in owned_houses:
                ownership_contrib = (effective_planet_score * self.OWNERSHIP_PERCENTAGE) / len(owned_houses)

            # Aspect contribution (20%, split equally) - uses effective score
            if house_num in aspected_houses:
                aspect_contrib = (effective_planet_score * self.ASPECT_PERCENTAGE) / len(aspected_houses)
    ...
    def aggregate_house_scores(
        ...
        """
        Aggregate contributions from all planets to calculate house scores (Phase 8 - Remove Normalization).

        REMOVED (Phase 8 Fix):
        - Cross-house normalization (was forcing sum to 100)
        - Division by total (was compressing magnitude)

        NEW APPROACH:
        - Pure accumulation of planet contributions
        - Soft cap at 100 per house
        - No forced distribution across houses
        ...
        # Sum contributions from all planets (pure accumulation)
        ...
        # Apply soft cap (no normalization)
        final_scores = {
            h: min(raw_score, 100.0)
    ...
    def calculate_house_activation(
        ...
        # Step 2: Calculate house contributions for each planet (Phase 8 - Transit Blending)
        planet_contributions = {}
        for planet, score_obj in planet_scores.scores.items():
            # Extract transit score from breakdown
            transit_score = score_obj.breakdown.transit
            ...
            contributions = self.calculate_planet_house_contributions(
                ...
                natal_chart,
                calculation_date,
                transit_score  # Phase 8: Pass transit score for blending
            )
