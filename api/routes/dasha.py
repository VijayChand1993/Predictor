...
class ActiveDashasRequest(BaseModel):
    ...
    chart_id: str = Field(..., description="Chart ID from natal chart generation", example="04ecf146-d0e1-4e72-8c30-fb8bba03e2e5")
...
async def get_active_dashas_get(
    ...
    target_date: Optional[date] = Query(None, description="Target date (defaults to today)", example=datetime.now().date())
):
    """
    Get active dasha periods for a specific date.
...
@router.get(
    "/{chart_id}/weight/all",
    summary="Get Dasha Weights for All Planets",
    description="Calculate dasha weights for all planets on a specific date"
)
async def get_all_dasha_weights(
    chart_id: str = PathParam(..., description="Chart ID", example="04ecf146-d0e1-4e72-8c30-fb8bba03e2e5"),
    target_date: Optional[date] = Query(None, description="Target date (defaults to today)", example=datetime.now().date())
):
    """
    Get dasha weights for all planets on a specific date.

    This endpoint calculates the dasha weight contribution for each of the 9 planets
    based on the active dasha periods (Mahadasha, Antardasha, Pratyantar, Sookshma).
    ...
    - date: Date for which weights are calculated
    - active_dashas: Current dasha periods
    - weights: Dictionary of planet weights (0-100 for each planet)
    """
    if target_date is None:
        target_date = date.today()
    ...
    try:
        ...
        # Get active dashas
        active_dashas = dasha_service.get_active_dashas(chart_json, target_date)

        # Calculate weights for all planets
        weights = {}
        for planet in Planet:
            weight = dasha_service.calculate_dasha_weight(planet, active_dashas)
            weights[planet.value] = weight
        ...
        raise HTTPException(status_code=500, detail=f"Error calculating weights: {str(e)}")
...
async def get_all_mahadashas(chart_id: str = PathParam(..., description="Chart ID", example="04ecf146-d0e1-4e72-8c30-fb8bba03e2e5")):
    """
